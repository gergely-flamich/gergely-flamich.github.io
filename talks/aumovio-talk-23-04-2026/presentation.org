...
- Distinguishability $\Leftrightarrow$ classification risk (such as error probability) between $Q_y$ and $R_y$
...
* Proposed divergence
$$
D_1(Q, P \mid \mathcal{P}) = \mathbb{E}_{f \sim \mathcal{P}} [\,|\mathbb{E}_{X \sim Q}[f(X)] - \mathbb{E}_{X \sim P}[f(X)]|\,]
$$
...
$$
#+ATTR_REVEAL: :frag (appear)
✅ When $\mathcal{P}$ is a GP, $D_1$ equivalent to MMD
